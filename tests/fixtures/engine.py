...
def setup_insurance_scenario(db_session: Session):
    ...
    version = EntityVersion(entity_id=entity.id, version_number=1, status=VersionStatus.PUBLISHED, changelog="V1")
...
def setup_dropdown_scenario(db_session):
    ...
    f_region = Field(
        entity_version_id=version.id, name="region", label="Region", data_type="string", sequence=1, is_free_value=False
    )
    ...
    f_city = Field(
        entity_version_id=version.id, name="city", label="City", data_type="string", sequence=2, is_free_value=False
    )
    ...
    r_milano = Rule(
        entity_version_id=version.id,
        target_field_id=f_city.id,
        target_value_id=v_milano.id,
        rule_type=RuleType.AVAILABILITY.value,
    ...
    return {"e_id": entity.id, "f_region": f_region.id, "f_city": f_city.id, "val_milano": v_milano.id}
...
def setup_operator_scenario(db_session):
    ...
    return {
        "entity_id": entity.id,
        "version_id": version.id,
        "fields": {"string": f_string.id, "number": f_number.id, "date": f_date.id, "dropdown": f_dropdown.id},
        "values": {"opt1": v_opt1.id, "opt2": v_opt2.id, "opt3": v_opt3.id},
    }
...
def setup_stress_scenario(db_session):
    ...
    r3 = Rule(
        ...
        conditions={"criteria": [{"field_id": fields[2].id, "operator": "GREATER_THAN", "value": "{{field_4}}"}]},
    )
...
def setup_sku_scenario(db_session):
    ...
    version = EntityVersion(
        entity_id=entity.id, version_number=1, status=VersionStatus.PUBLISHED, sku_base="LPT-PRO", sku_delimiter="-"
    )
    db_session.add(version)
    db_session.commit()
    ...
    return {
        "entity_id": entity.id,
        "version_id": version.id,
        "fields": {"cpu": f_cpu.id, "ram": f_ram.id, "gpu": f_gpu.id, "color": f_color.id, "notes": f_notes.id},
        "values": {
            "i5": v_i5.id,
            "i7": v_i7.id,
            "i9": v_i9.id,
...
def setup_sku_visibility_scenario(db_session):
    ...
    return {
        "entity_id": entity.id,
        "version_id": version.id,
        "fields": {"type": f_type.id, "cpu": f_cpu.id, "ram": f_ram.id},
        "values": {"desktop": v_desktop.id, "laptop": v_laptop.id, "i7": v_i7.id, "32g": v_32g.id},
    }
...
def setup_sku_hidden_default_scenario(db_session):
    ...
    return {
        "entity_id": entity.id,
        "version_id": version.id,
        "fields": {"cpu": f_cpu.id, "ram": f_ram.id},
        "values": {"i7": v_i7.id, "32g": v_32g.id},
    }
...
def setup_sku_availability_scenario(db_session):
    ...
    return {
        "entity_id": entity.id,
        "version_id": version.id,
        "fields": {"type": f_type.id, "ram": f_ram.id},
        "values": {"standard": v_standard.id, "premium": v_premium.id, "16g": v_16g.id, "32g": v_32g.id},
    }
...
def setup_calculation_scenario(db_session):
    ...
    version = EntityVersion(
        entity_id=entity.id, version_number=1, status=VersionStatus.PUBLISHED, sku_base="CFG", sku_delimiter="-"
    )
    db_session.add(version)
    db_session.commit()
    ...
    db_session.add_all(
        [r_calc_cooling, r_calc_support, r_edit_notes, r_avail_3y, r_calc_warranty, r_vis_status, r_mand_status]
    )
    db_session.commit()
