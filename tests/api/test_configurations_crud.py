...
def test_list_configurations_success(client: TestClient, auth_headers, config_scenario):
    ...
    # List configurations
    response = client.get(f"/configurations/?entity_version_id={config_scenario['version_id']}", headers=auth_headers)
...
def test_list_configurations_empty(client: TestClient, auth_headers, db_session):
    ...
    response = client.get(f"/configurations/?entity_version_id={version.id}", headers=auth_headers)
...
def test_update_configuration_success(client: TestClient, auth_headers, config_scenario):
    ...
    # Update config
    update_payload = {"name": "Updated Name", "data": [{"field_id": config_scenario["f_model_id"], "value": "Audi A6"}]}
