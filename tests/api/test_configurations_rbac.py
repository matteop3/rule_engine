...
def test_user_cannot_read_other_user_configuration(
...
def test_user_cannot_update_other_user_configuration(
    client: TestClient, config_scenario, user1_headers, user2_headers
):
    """
...
def test_user_cannot_delete_other_user_configuration(
    client: TestClient, config_scenario, user1_headers, user2_headers
):
    """
...
def test_author_cannot_read_other_author_configuration(
    client: TestClient, config_scenario, db_session
):
    """
...
def test_admin_can_read_any_user_configuration(
...
def test_list_configurations_user_sees_only_own(
    client: TestClient, config_scenario, user1_headers, user2_headers
):
    """
    ...
    assert all(config["name"] == "User1 Config" for config in configs1 if config["name"] in ["User1 Config", "User2 Config"])
    ...
    assert all(config["name"] == "User2 Config" for config in configs2 if config["name"] in ["User1 Config", "User2 Config"])
...
def test_list_configurations_admin_can_filter_by_user_id(
    ...
    assert all(config["name"] == "User1 Config" for config in configs if config["name"] in ["User1 Config", "Admin Config"])
...
def test_list_configurations_non_admin_cannot_filter_by_other_user_id(
    ...
    response = client.get(
        f"/configurations/?user_id={regular_user_2.id}",
        headers=user1_headers
    )
