...
class TestFinalizeBasicFunctionality:
    ...
    def test_finalize_changes_status(self, client, lifecycle_user_headers, draft_configuration):
    ...
    def test_finalize_returns_updated_config(self, client, lifecycle_user_headers, draft_configuration):
    ...
    def test_finalize_preserves_all_data(self, client, lifecycle_user_headers, draft_configuration):
    ...
    def test_finalize_preserves_version(self, client, lifecycle_user_headers, draft_configuration):
    ...
    def test_finalize_preserves_is_complete(self, client, lifecycle_user_headers, draft_configuration):
...
class TestFinalizeIdempotencyAndConstraints:
    ...
    def test_finalize_already_finalized(self, client, lifecycle_user_headers, finalized_configuration):
    ...
    def test_finalize_deleted_config_accessible_but_finalized(
        ...
        response = client.post(
            f"/configurations/{soft_deleted_configuration.id}/finalize", headers=lifecycle_admin_headers
        )
    ...
    def test_finalize_updates_audit_fields(
        self, client, db_session, lifecycle_user_headers, draft_configuration, lifecycle_user
    ):
    ...
    def test_finalize_sets_updated_at(self, client, lifecycle_user_headers, draft_configuration):
...
class TestFinalizeAccessControl:
    ...
    def test_finalize_owner_can_finalize(self, client, lifecycle_user_headers, draft_configuration):
        """Owner can finalize their own configuration."""
        response = client.post(f"/configurations/{draft_configuration.id}/finalize", headers=lifecycle_user_headers)

        assert response.status_code == 200

    def test_finalize_admin_can_finalize_any(self, client, lifecycle_admin_headers, draft_configuration):
    ...
    def test_finalize_user_cannot_finalize_others(self, client, second_lifecycle_user_headers, draft_configuration):
    ...
    def test_finalize_author_cannot_finalize_others(self, client, lifecycle_author_headers, draft_configuration):
        """AUTHOR cannot finalize other user's configuration."""
        response = client.post(f"/configurations/{draft_configuration.id}/finalize", headers=lifecycle_author_headers)

        assert response.status_code == 403

    def test_finalize_without_auth(self, client, draft_configuration):
        """Finalize without authentication should return 401."""
        response = client.post(f"/configurations/{draft_configuration.id}/finalize")
...
class TestFinalizeEdgeCases:
    ...
    def test_finalize_nonexistent_config(self, client, lifecycle_user_headers):
        """Finalizing non-existent config should return 404."""
        response = client.post(
            "/configurations/00000000-0000-0000-0000-000000000000/finalize", headers=lifecycle_user_headers
        )
    ...
    def test_finalize_with_incomplete_config_rejected(
        ...
        response = client.post(
            f"/configurations/{configuration_with_empty_data.id}/finalize", headers=lifecycle_user_headers
        )

        assert response.status_code == 400
        assert "incomplete configuration" in response.json()["detail"]

    def test_finalize_then_update_blocked(self, client, lifecycle_user_headers, draft_configuration):
    ...
    def test_finalize_then_upgrade_blocked(self, client, lifecycle_user_headers, draft_configuration):
        ...
        upgrade_response = client.post(
            f"/configurations/{draft_configuration.id}/upgrade", headers=lifecycle_user_headers
        )

        assert upgrade_response.status_code == 409

    def test_finalize_then_clone_allowed(self, client, lifecycle_user_headers, draft_configuration):
    ...
    def test_finalize_then_delete_blocked_for_user(self, client, lifecycle_user_headers, draft_configuration):
        ...
        # Then try to delete
        delete_response = client.delete(f"/configurations/{draft_configuration.id}", headers=lifecycle_user_headers)

        assert delete_response.status_code == 403

    def test_finalize_then_read_allowed(self, client, lifecycle_user_headers, draft_configuration):
    ...
    def test_finalize_persists_to_database(self, client, db_session, lifecycle_user_headers, draft_configuration):
...
class TestFinalizeWorkflows:
    ...
    def test_create_update_finalize_workflow(self, client, lifecycle_user_headers, published_version_for_lifecycle):
        ...
        # Update
        update_response = client.patch(
            f"/configurations/{config_id}", json={"name": "Updated Workflow Config"}, headers=lifecycle_user_headers
        )
    ...
    def test_finalize_clone_modify_workflow(
        ...
        original_response = client.get(f"/configurations/{draft_configuration.id}", headers=lifecycle_user_headers)
