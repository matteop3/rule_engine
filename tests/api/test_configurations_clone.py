...
class TestCloneBasicFunctionality:
    ...
    def test_clone_draft_creates_new_config(
    ...
    def test_clone_finalized_creates_new_config(
    ...
    def test_clone_result_always_draft(
        self, client, lifecycle_user_headers, finalized_configuration
    ):
    ...
    def test_clone_copies_input_data(
        self, client, lifecycle_user_headers, draft_configuration
    ):
    ...
    def test_clone_copies_version_reference(
        self, client, lifecycle_user_headers, draft_configuration
    ):
    ...
    def test_clone_copies_name_with_suffix(
        self, client, lifecycle_user_headers, draft_configuration
    ):
    ...
    def test_clone_null_name_stays_null(
        self, client, lifecycle_user_headers, configuration_null_name
    ):
        """Cloning config with null name should result in null name."""
        response = client.post(
            f"/configurations/{configuration_null_name.id}/clone",
            headers=lifecycle_user_headers
        )
    ...
    def test_clone_copies_is_complete(
        self, client, lifecycle_user_headers, draft_configuration
    ):
    ...
    def test_clone_source_unchanged(
    ...
    def test_clone_returns_source_id(
        self, client, lifecycle_user_headers, draft_configuration
    ):
...
class TestCloneAccessControl:
    ...
    def test_clone_owner_can_clone_own(
        self, client, lifecycle_user_headers, draft_configuration
    ):
    ...
    def test_clone_admin_can_clone_any(
    ...
    def test_clone_user_cannot_clone_others(
    ...
    def test_clone_author_cannot_clone_others(
    ...
    def test_clone_deleted_config_still_accessible_to_owner(
        ...
        response = client.post(
            f"/configurations/{soft_deleted_configuration.id}/clone",
            headers=lifecycle_admin_headers
        )
...
class TestCloneEdgeCases:
    ...
    def test_clone_assigns_current_user_as_owner(
        self, client, db_session, lifecycle_admin_headers, draft_configuration,
        lifecycle_admin
    ):
        ...
        # Verify ownership
        clone = db_session.query(Configuration).filter(
            Configuration.id == clone_id
        ).first()
        assert clone.user_id == lifecycle_admin.id
        assert clone.user_id != draft_configuration.user_id

    def test_clone_sets_new_audit_timestamps(
        self, client, lifecycle_user_headers, finalized_configuration
    ):
    ...
    def test_clone_nonexistent_config(
        ...
        response = client.post(
            "/configurations/00000000-0000-0000-0000-000000000000/clone",
            headers=lifecycle_user_headers
        )

        assert response.status_code == 404

    def test_clone_without_auth(
    ...
    def test_clone_is_deleted_false(
        self, client, lifecycle_user_headers, draft_configuration
    ):
    ...
    def test_multiple_clones_have_unique_ids(
        self, client, lifecycle_user_headers, draft_configuration
    ):
