...
class TestDraftToDraftTransitions:
    ...
    def test_draft_update_stays_draft(
    ...
    def test_draft_upgrade_stays_draft(
...
class TestDraftToFinalizedTransitions:
    ...
    def test_draft_finalize_to_finalized(
        self, client, lifecycle_user_headers, draft_configuration
    ):
...
class TestDraftToDeletedTransitions:
    ...
    def test_draft_delete_hard_delete(
...
class TestFinalizedToFinalizedBlockedTransitions:
    ...
    def test_finalized_update_blocked(
    ...
    def test_finalized_upgrade_blocked(
        self, client, lifecycle_user_headers, finalized_configuration
    ):
    ...
    def test_finalized_finalize_blocked(
        self, client, lifecycle_user_headers, finalized_configuration
    ):
...
class TestFinalizedToSoftDeletedTransitions:
    ...
    def test_finalized_delete_user_denied(
        self, client, lifecycle_user_headers, finalized_configuration
    ):
...
class TestCloneTransitions:
    ...
    def test_clone_draft_creates_draft(
        self, client, lifecycle_user_headers, draft_configuration
    ):
...
class TestStateTransitionMatrix:
    ...
    def test_all_draft_operations(
        ...
        published_version_for_lifecycle, lifecycle_user
    ):
        ...
        finalize_resp = client.post(
            f"/configurations/{config2.id}/finalize",
            headers=lifecycle_user_headers
        )
        ...
        delete_resp = client.delete(
            f"/configurations/{config3_id}",
            headers=lifecycle_user_headers
        )
        assert delete_resp.status_code == 204
        db_session.expire_all()
        assert db_session.query(Configuration).filter(
            Configuration.id == config3_id
        ).first() is None

        # CLONE: DRAFT -> new DRAFT
        config4 = create_draft()
        clone_resp = client.post(
            f"/configurations/{config4.id}/clone",
            headers=lifecycle_user_headers
        )
    ...
    def test_all_finalized_operations(
        self, client, db_session, lifecycle_user_headers, lifecycle_admin_headers,
        published_version_for_lifecycle, lifecycle_user, lifecycle_admin
    ):
        ...
        upgrade_resp = client.post(
            f"/configurations/{config2.id}/upgrade",
            headers=lifecycle_user_headers
        )
        ...
        finalize_resp = client.post(
            f"/configurations/{config3.id}/finalize",
            headers=lifecycle_user_headers
        )
        ...
        delete_admin_resp = client.delete(
            f"/configurations/{config5_id}",
            headers=lifecycle_admin_headers
        )
        assert delete_admin_resp.status_code == 204
        db_session.expire_all()
        soft_deleted = db_session.query(Configuration).filter(
            Configuration.id == config5_id
        ).first()
        ...
        clone_resp = client.post(
            f"/configurations/{config6.id}/clone",
            headers=lifecycle_user_headers
        )
        assert clone_resp.status_code == 201
        assert clone_resp.json()["status"] == "DRAFT"
