...
class TestUpgradeBasicFunctionality:
    ...
    def test_upgrade_updates_version_id(
        self, client, db_session, lifecycle_user_headers, configuration_on_archived_version, multi_version_entity
    ):
        ...
        response = client.post(f"/configurations/{config.id}/upgrade", headers=lifecycle_user_headers)
    ...
    def test_upgrade_recalculates_is_complete(
        self, client, db_session, lifecycle_user_headers, configuration_on_published_multi_version, multi_version_entity
    ):
    ...
    def test_upgrade_already_on_latest(
        self, client, lifecycle_user_headers, configuration_on_published_multi_version, multi_version_entity
    ):
...
class TestUpgradeStatusConstraints:
    ...
    def test_upgrade_draft_allowed(self, client, lifecycle_user_headers, configuration_on_archived_version):
    ...
    def test_upgrade_finalized_blocked(self, client, lifecycle_user_headers, finalized_configuration):
    ...
    def test_upgrade_finalized_error_message(self, client, lifecycle_user_headers, finalized_configuration):
    ...
    def test_upgrade_deleted_config_accessible(self, client, lifecycle_admin_headers, soft_deleted_configuration):
        """Soft-deleted config is accessible but blocked because it's FINALIZED."""
        response = client.post(
            f"/configurations/{soft_deleted_configuration.id}/upgrade", headers=lifecycle_admin_headers
        )
...
class TestUpgradeAccessControl:
    ...
    def test_upgrade_owner_can_upgrade_own(self, client, lifecycle_user_headers, configuration_on_archived_version):
    ...
    def test_upgrade_admin_can_upgrade_any(self, client, lifecycle_admin_headers, configuration_on_archived_version):
    ...
    def test_upgrade_user_cannot_upgrade_others(
        ...
        response = client.post(
            f"/configurations/{configuration_on_archived_version.id}/upgrade", headers=second_lifecycle_user_headers
        )

        assert response.status_code == 403

    def test_upgrade_without_auth(self, client, configuration_on_archived_version):
        """Upgrade without authentication should return 401."""
        response = client.post(f"/configurations/{configuration_on_archived_version.id}/upgrade")
...
class TestUpgradeEdgeCases:
    ...
    def test_upgrade_updates_audit_fields(
        self, client, db_session, lifecycle_user_headers, configuration_on_archived_version, lifecycle_user
    ):
    ...
    def test_upgrade_nonexistent_config(self, client, lifecycle_user_headers):
        """Upgrading non-existent config should return 404."""
        response = client.post(
            "/configurations/00000000-0000-0000-0000-000000000000/upgrade", headers=lifecycle_user_headers
        )

        assert response.status_code == 404

    def test_upgrade_idempotent_when_already_latest(
        self, client, db_session, lifecycle_user_headers, configuration_on_published_multi_version
    ):
        """Multiple upgrade calls should be safe when already on latest."""
        config_id = configuration_on_published_multi_version.id

        # First upgrade
        response1 = client.post(f"/configurations/{config_id}/upgrade", headers=lifecycle_user_headers)
        assert response1.status_code == 200
        version1 = response1.json()["entity_version_id"]

        # Second upgrade (should be no-op)
        response2 = client.post(f"/configurations/{config_id}/upgrade", headers=lifecycle_user_headers)
    ...
    def test_upgrade_preserves_name(self, client, lifecycle_user_headers, configuration_on_archived_version):
    ...
    def test_upgrade_preserves_status_draft(self, client, lifecycle_user_headers, configuration_on_archived_version):
