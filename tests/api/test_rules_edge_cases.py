...
from app.models.domain import Field, FieldType
...
class TestRuleEdgeCases:
    ...
    def test_rule_description_optional(self, client: TestClient, admin_headers, db_session, draft_version):
    ...
    def test_multiple_rules_same_target(self, client: TestClient, admin_headers, db_session, draft_version):
        ...
        # Verify both exist
        list_response = client.get(f"/rules/?entity_version_id={draft_version.id}", headers=admin_headers)
        rules = [r for r in list_response.json() if r["target_field_id"] == target_field.id]
        assert len(rules) == 2

    def test_rule_with_multiple_criteria(self, client: TestClient, admin_headers, db_session, draft_version):
