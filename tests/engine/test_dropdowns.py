...
from app.models.domain import Entity, EntityVersion, Field, Rule, RuleType, Value, VersionStatus
...
def test_dropdown_cascade_logic(db_session, setup_dropdown_scenario):
    ...
    payload = CalculationRequest(
        entity_id=ids["e_id"], current_state=[FieldInputState(field_id=ids["f_region"], value="NORD")]
    )
    ...
    payload = CalculationRequest(
        entity_id=ids["e_id"], current_state=[FieldInputState(field_id=ids["f_region"], value="SUD")]
    )
    resp = service.calculate_state(db_session, payload)
    f_city = next(f for f in resp.fields if f.field_id == ids["f_city"])
