...
from app.models.domain import Entity, EntityVersion, Field, Rule, RuleType, VersionStatus
...
def setup_stress_scenario(db_session):
    ...
    # Sequence: 1, 2, 3
    f_a = Field(
        entity_version_id=version.id, name="A", label="Driver", data_type="string", sequence=1, is_free_value=True
    )
    f_b = Field(
        entity_version_id=version.id, name="B", label="Middle", data_type="string", sequence=2, is_free_value=True
    )
    f_c = Field(
        entity_version_id=version.id, name="C", label="Tail", data_type="string", sequence=3, is_free_value=True
    )

    # --- FIELDS FOR SEQUENCE CHECK (X, Y) ---
    # X (Seq 10) depends on Y (Seq 20)
    f_x = Field(
        entity_version_id=version.id, name="X", label="Early", data_type="string", sequence=10, is_free_value=True
    )
    f_y = Field(
        entity_version_id=version.id, name="Y", label="Late", data_type="string", sequence=20, is_free_value=True
    )

    db_session.add_all([f_a, f_b, f_c, f_x, f_y])
    db_session.commit()

    return {"v_id": version.id, "e_id": entity.id, "A": f_a.id, "B": f_b.id, "C": f_c.id, "X": f_x.id, "Y": f_y.id}
