...
class TestRBACEndToEnd:
    ...
    def test_user_role_can_only_use_published_versions(self, client: TestClient, admin_headers, user_headers):
        """
        E2E: USER role can only calculate on PUBLISHED versions.
        """
        # Admin creates entity and version
        entity_resp = client.post(
            "/entities/", json={"name": "RBAC Test Entity", "description": "RBAC test"}, headers=admin_headers
        )
        entity_id = entity_resp.json()["id"]

        version_resp = client.post(
            "/versions/", json={"entity_id": entity_id, "changelog": "Draft"}, headers=admin_headers
        )
        ...
        calc_draft = client.post(
            "/engine/calculate",
            json={"entity_id": entity_id, "entity_version_id": version_id, "current_state": []},
        ...
        calc_published = client.post(
            "/engine/calculate", json={"entity_id": entity_id, "current_state": []}, headers=user_headers
        )
        assert calc_published.status_code == 200

    def test_author_can_preview_draft_via_engine(self, client: TestClient, admin_headers, author_headers):
        """
        E2E: AUTHOR role can calculate on DRAFT versions for preview.
        """
        # Admin creates entity
        entity_resp = client.post(
            "/entities/", json={"name": "Author Preview Test", "description": "Author preview"}, headers=admin_headers
        )
        entity_id = entity_resp.json()["id"]

        # Author creates version (authors can create versions)
        version_resp = client.post(
            "/versions/", json={"entity_id": entity_id, "changelog": "Author draft"}, headers=author_headers
        )
        version_id = version_resp.json()["id"]
