...
from app.models.domain import Configuration, ConfigurationStatus, EntityVersion, Field, User, UserRole, VersionStatus
...
def calculate_configuration_state(
    db: Session, engine_service: RuleEngineService, version: EntityVersion, data: list[dict[str, Any]]
) -> CalculationResponse:
    """
    ...
    try:
        ...
        calc_request = CalculationRequest(
            entity_id=version.entity_id, entity_version_id=version.id, current_state=current_state_objects
        )
...
def create_configuration(
    ...
    calc_result: CalculationResponse = calculate_configuration_state(
        db=db, engine_service=engine_service, version=version, data=data_list
    )
...
def update_configuration(
    ...
    if "data" in update_data:
        ...
        calc_result: CalculationResponse = calculate_configuration_state(
            db=db, engine_service=engine_service, version=version, data=update_data["data"]
        )
...
def upgrade_configuration(
    ...
    calc_result = calculate_configuration_state(
        db=db, engine_service=engine_service, version=latest_version, data=config.data
    )
...
def finalize_configuration(
    config_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """
